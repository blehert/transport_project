...
def extract_nmea_from_log(
    ...
    # Оставили только строки с NMEA, столбцы с датой, временем, NMEA
    geo = geo[geo[4] == ' geo '][[0, 1, 5]]
...
def nmea_coordinates_to_degrees(nmea_array):
    ...
    # Перевод из минут в десятичную дробь *100/60
    float_degrees = nmea_minutes * 5 / 3
...
def draw_polyline_trip_track(my_map,
    ...
    folium.PolyLine(
        ...
        tooltip=folium.Tooltip(
            tooltip_text,
            sticky=False,  # не прилипает к курсору
            permanent=False,  # отображается всегда
            #          style='background-color:grey'
        )
    ).add_to(my_map)
...
def log2html(
    ...
    # Включение отображения слоев на карте
